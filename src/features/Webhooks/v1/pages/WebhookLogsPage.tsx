...
const LOG_STATUS_DOTS: Record<string, string> = { all: "bg-gray-400", success: "bg-emerald-500", failed: "bg-red-500" };
...
export default function WebhookLogsPage() {
  ...
  return (
    <div className="flex h-full w-full flex-col overflow-auto cd-page" style={{ backgroundColor: "var(--cd-bg)" }}>
      ...
      >
        <div className="flex items-center gap-3 mb-2 text-sm font-medium" style={{ color: "var(--cd-text-muted)" }}>
          <Link to="/org/dashboard/webhooks" className="hover:text-[var(--cd-text)] transition-colors">Webhooks</Link>
          <span>/</span>
          <Link to={`/org/dashboard/webhooks/${id}`} className="hover:text-[var(--cd-text)] transition-colors">{webhook?.name || 'Loading...'}</Link>
          <span>/</span>
          <span style={{ color: "var(--cd-text)" }}>Logs</span>
        </div>
        <div className="flex items-center gap-4 mt-2">
          <button onClick={() => navigate(-1)} className="p-2 rounded-lg transition-colors hover:bg-[var(--cd-hover)]" style={{ color: "var(--cd-text-muted)" }}>
            <ArrowLeft size={20} />
          </button>
          <div>
      ...
      {/* Filter Bar */}
      <div className="border-b px-5 py-3 sm:px-8 lg:px-10 flex gap-4" style={{ backgroundColor: "var(--cd-surface-2)", borderColor: "var(--cd-border-subtle)" }}>
      ...
      <div className="flex-1 p-5 sm:p-8 lg:p-10 flex flex-col gap-4">
        ...
        <div className="hidden md:block">
          <div className="rounded-xl border overflow-hidden shadow-sm" style={{ backgroundColor: "var(--cd-surface)", borderColor: "var(--cd-border)" }}>
            <LogsTable 
              logs={logs} 
              isLoading={isLoading} 
              onRetry={handleRetry}
