...
export default function TaskRow({ task, onDelete }: Props) {
  ...
  return (
    ...
    >
      ...
      <td className="min-w-0 py-3.5 pl-5 pr-5">
        <div className="flex max-w-[520px] flex-col gap-2">
          ...
          <div className="min-w-0">
            <span className="block w-full truncate text-sm font-semibold transition-colors group-hover:text-[var(--cd-primary)]" style={{ color: "var(--cd-text)" }}>
              {task.title}
            </span>
            <span className="mt-1 block w-full overflow-hidden text-ellipsis whitespace-nowrap text-xs leading-5" style={{ color: "var(--cd-text-muted)" }}>
              {task.description}
            </span>
          </div>
        </div>
      ...
      <td className="py-3.5 pr-5"><PriorityBadge priority={task.priority} size="sm" /></td>
      ...
      <td className="py-3.5 pr-5">
        <div className="flex flex-col gap-1.5">
          <span className={`inline-flex w-fit items-center gap-1 rounded-md px-2 py-1 text-[10px] font-medium ${subStatusCfg.bg} ${subStatusCfg.text}`}>
