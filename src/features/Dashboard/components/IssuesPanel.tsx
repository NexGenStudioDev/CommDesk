    ...
    <div className="cd-card cd-card-hover">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div
          ...
          style={{ backgroundColor: theme.danger.subtle, border: `1px solid ${theme.border.subtle}` }}
          onClick={() => alert("View open issues")}
        >
          <AlertCircle size={17} style={{ color: theme.danger.default }} />
          <div>
            <p className="text-xs" style={{ color: theme.text.secondary }}>Open</p>
            <p className="text-lg font-semibold" style={{ color: theme.danger.default }}>{data.open}</p>
          </div>
        </div>

        <div
          className="p-3 rounded-xl flex items-center gap-2 cursor-pointer transition-all hover:-translate-y-[1px]"
          style={{ backgroundColor: theme.success.subtle, border: `1px solid ${theme.border.subtle}` }}
          onClick={() => alert("View resolved issues")}
        >
          <CheckCircle size={17} style={{ color: theme.success.default }} />
          <div>
            <p className="text-xs" style={{ color: theme.text.secondary }}>Resolved</p>
            <p className="text-lg font-semibold" style={{ color: theme.success.default }}>{data.resolved}</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <span>
          {hasIssues ? `${data.open} issue(s) need attention` : "All issues are resolved — great job!"}
        </span>
      </div>
    </div>
  );
